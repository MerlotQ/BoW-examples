  ...
  DetectionResult &match)
{
  ...
  {
    int max_id = (int)entry_id - m_params.dislocal;//远距离回环，最近的dislocal个图像不检测
    ...
    if(!qret.empty())
    {
      ...
      if(!m_params.use_nss || ns_factor >= m_params.min_nss_factor)
      {
        ...
        // (ret is sorted in descending score order now) score递减
        ...
        if(!qret.empty())
        {
          ...
          computeIslands(qret, islands); //关键，计算island
          ...
          if(!islands.empty())
          {
            const tIsland& island = 
              *std::max_element(islands.begin(), islands.end());//获得得分最高的island
            
            // check temporal consistency of this island
            updateTemporalWindow(island, entry_id);
            
            // get the best candidate (maybe match)
            match.match = island.best_entry;//以best island中best score的image为匹配图像
  ...
  (QueryResults &q, vector<tIsland> &islands) const
{
  ...
  {
    // sort query results in ascending order of ids按照递增为query results 排序
      ...
      else
      {
        ...
        if(length >= m_params.min_matches_per_group)
        {
          islands.push_back( tIsland(first_island_entry, last_island_entry,
            calculateIslandScore(q, i_first, i_last)) );//计算island的score
          
          islands.back().best_score = best_score;//记录当前island中图像相似度最高的score
          islands.back().best_entry = best_entry;//记录当前island中图像相似度最高的图像id
        }
